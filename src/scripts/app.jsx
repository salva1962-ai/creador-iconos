...
const UI_ICONS = {
    ...
    Content: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
    Mockups: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>,
    Effects: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
...
const PASTEL_COLORS = [
    ...
    "#FFB3D9", "#FFC9B3", "#B3FFB3", "#B3E5FF", "#E1B3FF", "#FFE1B3"
];
...
const GRADIENT_PALETTES = [
    ...
    { name: "Purplex", colors: ["#614ad3", "#e42c64"] },
    { name: "Cyber", colors: ["#00f2fe", "#4facfe"] },
    { name: "Midnight", colors: ["#232526", "#414345"] }
];

const BRAND_GRADIENTS = [
    { name: "Ocean", colors: ["#0ea5e9", "#2563eb"] },
    { name: "Sunset", colors: ["#f43f5e", "#fb923c"] },
    { name: "Forest", colors: ["#10b981", "#059669"] },
    { name: "Royal", colors: ["#8b5cf6", "#d946ef"] },
    { name: "Cyber", colors: ["#06b6d4", "#8b5cf6"] },
    { name: "Industrial", colors: ["#334155", "#0f172a"] }
];
...
const PRO_FONTS = [
    ...
    { id: "JetBrains Mono", name: "Code", family: "'JetBrains Mono', monospace" }
];

const PRESETS = [
    { name: "Midnight Neon", template: "neon-outer", shape: "hexagon", primary: "#8b5cf6", secondary: "#ec4899", noise: 5, gloss: false, pattern: "none" },
    { name: "Glass Apple", template: "inner-glow", shape: "rounded-square", primary: "#ffffff", secondary: "#f8fafc", noise: 0, glass: true, pattern: "none" },
    { name: "Gold 3D", template: "isometric-3d", shape: "hexagon", primary: "#fbbf24", secondary: "#d97706", gloss: true, noise: 0, pattern: "dots" },
    { name: "Retro Mesh", template: "mesh-gradient", shape: "circle", primary: "#f472b6", secondary: "#60a5fa", noise: 12, glass: false, pattern: "grid" }
];

const MOCKUP_TEMPLATES = [
    { id: "none", name: "Solo Logo", icon: "🎨" },
    { id: "iphone", name: "iPhone App", icon: "📱" },
    { id: "macos", name: "macOS Dock", icon: "💻" },
    { id: "browser", name: "Favicon Preview", icon: "🌐" },
    { id: "social", name: "Profile Pic", icon: "👤" }
];

const PATTERNS = [
    { id: "none", name: "Liso", path: "" },
    { id: "dots", name: "Puntos", path: '<pattern id="pattern-dots" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="white" opacity="0.3" /></pattern>' },
    { id: "lines", name: "Líneas", path: '<pattern id="pattern-lines" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="1" height="10" fill="white" opacity="0.2" /></pattern>' },
    { id: "grid", name: "Cuadrícula", path: '<pattern id="pattern-grid" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse"><path d="M 10 0 L 0 0 0 10" fill="none" stroke="white" stroke-width="0.5" opacity="0.2" /></pattern>' }
];
...
const SHAPES = [
    { id: "circle", name: "Círculo", path: "M 50, 50 m -40, 0 a 40,40 0 1,0 80,0 a 40,40 0 1,0 -80,0" },
    { id: "square", name: "Cuadrado", path: "M 10,10 H 90 V 90 H 10 Z" },
    { id: "rounded-square", name: "Rounded", path: "M 30,10 H 70 A 20,20 0 0 1 90,30 V 70 A 20,20 0 0 1 70,90 H 30 A 20,20 0 0 1 10,70 V 30 A 20,20 0 0 1 30,10 Z" },
    { id: "hexagon", name: "Hexágono", path: "M 50,10 L 90,30 V 70 L 50,90 L 10,70 V 30 Z" }
];
...
const IconStudio = () => {
    ...
    const contrastInfo = useMemo(() => {
        ...
        return {
            isGood: brightness < 200, // Si es muy claro, el blanco no se verá bien
    ...
    const runBatchExport = async () => {
        ...
        for (let i = 0; i < EXPORT_CONFIG.length; i++) {
            const { size, name } = EXPORT_CONFIG[i];
...
<meta name="theme-color" content="${primaryColor}">`;
    ...
    return (
        <div className="flex h-screen w-full overflow-hidden font-sans selection:bg-pink-200">
            {/* Sidebar */}
            <aside className="w-[380px] h-full glass-panel border-r border-slate-200 dark:border-slate-800 z-20 flex flex-col shadow-2xl">
                <div className="p-8 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <h1 className="text-2xl font-black tracking-tight flex items-center gap-3 text-slate-900 dark:text-white">
                        <span className="bg-gradient-to-br from-pink-500 to-violet-600 w-10 h-10 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-pink-500/20">
                            I
                        </span>
                        IconStudio <span className="text-[10px] uppercase tracking-widest bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-full">Pro</span>
                    </h1>
                    <button 
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-4">
                        {Object.entries(UI_ICONS).map(([name, icon]) => (
                            <button 
                                key={name}
                            ...
                            >
                                {icon}
                                <span className="text-[10px] mt-1 font-bold">{name}</span>
                            </button>
                        ))}
                    </div>
                    ...
                    {activeSection === "Style" && (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Patrones de Fondo</label>
                                <div className="grid grid-cols-4 gap-2 mb-6">
                                    {PATTERNS.map(p => (
                                        ...
                                        >
                                            <div className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-700 overflow-hidden relative">
                                                <svg viewBox="0 0 40 40" className="w-full h-full">
                                                    <defs dangerouslySetInnerHTML={{ __html: p.path.replace('id="pattern-', `id="preview-${p.id}`) }} />
                                                    <rect width="40" height="40" fill={p.id !== "none" ? `url(#preview-${p.id})` : "transparent"} />
                                                </svg>
                                            </div>
                                            <span className="text-[9px] font-bold uppercase tracking-tighter">{p.name}</span>
                                        </button>
                                    ))}
                                </div>

                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Presets Rápidos</label>
                                ...
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Plantillas Maestras</label>
                                <div className="grid grid-cols-1 gap-4">
                                    {ADVANCED_TEMPLATES.map(tpl => (
                                        <button
                                            key={tpl.id}
                                            onClick={() => setSelectedTemplate(tpl.id)}
                                            className={`group relative p-4 rounded-3xl border-2 transition-all flex items-start gap-4 ${
                                        ...
                                        >
                                            <div className="w-16 h-16 rounded-2xl overflow-hidden bg-white dark:bg-slate-950 flex-shrink-0 shadow-inner" 
                                                 dangerouslySetInnerHTML={{ __html: tpl.preview(primaryColor, secondaryColor) }} />
                                            <div className="text-left">
                                                <div className={`font-bold text-sm ${selectedTemplate === tpl.id ? "text-pink-600" : "text-slate-900 dark:text-white"}`}>
                                                    {tpl.name}
                                                </div>
                            ...
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Paleta Primaria</label>
                                ...
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Gradients Reales (Nivel Pro)</label>
                                <div className="grid grid-cols-3 gap-2 mb-6">
                                    {GRADIENT_PALETTES.map(p => (
                                        <button 
                                            key={p.name}
                                            onClick={() => { setPrimaryColor(p.colors[0]); setSecondaryColor(p.colors[1]); }}
                                        ...
                                        >
                                            <div className="absolute inset-0 opacity-80 group-hover:opacity-100 transition-opacity" style={{ background: `linear-gradient(135deg, ${p.colors[0]}, ${p.colors[1]})` }} />
                                            <span className="relative z-10 text-[8px] font-black text-white uppercase drop-shadow-md">{p.name}</span>
                                        </button>
                                    ))}
                                </div>

                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Brand Gradients</label>
                                <div className="grid grid-cols-3 gap-2 mb-6">
                                    {BRAND_GRADIENTS.map(g => (
                                        <button
                                            key={g.name}
                                            onClick={() => { setPrimaryColor(g.colors[0]); setSecondaryColor(g.colors[1]); }}
                                            className="h-12 rounded-2xl flex flex-col items-center justify-center gap-1 border border-slate-200 dark:border-slate-800 hover:scale-105 transition-all overflow-hidden group relative"
                                        >
                                            <div className="absolute inset-0 opacity-80 group-hover:opacity-100 transition-opacity" style={{ background: `linear-gradient(135deg, ${g.colors[0]}, ${g.colors[1]})` }} />
                                            <span className="relative z-10 text-[8px] font-black text-white uppercase drop-shadow-md">{g.name}</span>
                                        </button>
                                    ))}
                                </div>

                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Modos de Color</label>
                                <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-3xl space-y-4">
                                    ...
                                    <div className="pt-2 border-t border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-2">
                                        ...
                                        <button onClick={() => { setPrimaryColor(secondaryColor); setSecondaryColor(primaryColor); }} className="py-2 rounded-xl bg-white dark:bg-slate-800 text-[9px] font-bold text-slate-500 uppercase">Invertir</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {activeSection === "Content" && (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Forma del Contenedor</label>
                                <div className="grid grid-cols-4 gap-2">
                                    {SHAPES.map(s => (
                                        ...
                                        >
                                            <svg className="w-6 h-6 mb-1" viewBox="0 0 100 100">
                                                <path d={s.path} fill="currentColor" opacity="0.8" />
                                            </svg>
                                            <span className="text-[9px] font-bold uppercase">{s.name}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Icono / Símbolo</label>
                                ...
                                <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-[32px] mb-6 space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] font-black text-pink-500 bg-pink-500/10 px-2 py-1 rounded-lg">{fontSize}px</span>
                                    </div>
                                    <input 
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-center">
                                                <label className="text-[9px] font-black uppercase text-slate-400">Posición X</label>
                                                <span className="text-[9px] font-bold text-indigo-500">{textOffsetX}</span>
                                            </div>
                                            <input 
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-center">
                                                <label className="text-[9px] font-black uppercase text-slate-400">Posición Y</label>
                                                <span className="text-[9px] font-bold text-indigo-500">{textOffsetY}</span>
                                            </div>
                                            <input 
                                ...
                                <div className="p-6 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[32px] text-white shadow-xl mb-6">
                                    <div className="flex items-center justify-between mb-4">
                                        <label className="text-xs font-black uppercase tracking-widest">Símbolo Extra / Decoración</label>
                                        {secondarySymbol && (
                                            <button onClick={() => setSecondarySymbol("")} className="text-[10px] bg-white/20 px-2 py-1 rounded-lg font-bold hover:bg-white/30">ELIMINAR</button>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mb-4">
                                        {[
                                            { label: "Encima", x: 0, y: -25 },
                                            { label: "Debajo", x: 0, y: 25 },
                                            { label: "Izq", x: -25, y: 0 },
                                            { label: "Der", x: 25, y: 0 }
                                        ].map(pos => (
                                            <button
                                                key={pos.label}
                                                onClick={() => { setSecOffsetX(pos.x); setSecOffsetY(pos.y); }}
                                ...
                                <div className="flex items-center gap-2 mb-4 p-1 bg-slate-100 dark:bg-slate-900 rounded-2xl">
                                    <button 
                                ...
                                {Object.entries(SYMBOL_CATEGORIES).map(([cat, symbols]) => (
                                    <div key={cat} className="mb-4">
                                        <span className="text-[10px] font-bold text-slate-400 uppercase mb-2 block">{cat}</span>
                                        <div className="flex flex-wrap gap-2">
                                            {symbols.map(s => (
                                                <button 
                                                    key={s} 
                                                    onClick={() => activeLayer === "main" ? setIconText(s) : setSecondarySymbol(s)}
                                                    className={`w-10 h-10 rounded-xl bg-white dark:bg-slate-800 shadow-sm hover:shadow-md hover:scale-110 transition-all flex items-center justify-center text-xl ${
                                                        (activeLayer === "main" ? iconText === s : secondarySymbol === s) ? "ring-2 ring-pink-500" : ""
                                                    }`}
                                                >
                                                    {s}
                                                </button>
                            ...
                            <div className="p-6 bg-indigo-500/10 border-2 border-indigo-500/20 rounded-[32px] relative overflow-hidden group">
                                <label className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 mb-3 block">Subir Imagen</label>
                                <input type="file" onChange={handleFileUpload} className="hidden" id="file-upload" />
                                <label htmlFor="file-upload" className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-indigo-400/50 rounded-2xl cursor-pointer hover:bg-indigo-500/5 transition-all">
                                    <svg className="w-8 h-8 text-indigo-500 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                    ...
                    {activeSection === "Mockups" && (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Visualización en Dispositivo</label>
                            <div className="grid grid-cols-2 gap-4">
                                {MOCKUP_TEMPLATES.map(m => (
                                    ...
                                    >
                                        ...
                                        <span className={`text-[10px] font-black uppercase tracking-widest ${selectedMockup === m.id ? "text-pink-600" : "text-slate-500"}`}>
                                            {m.name}
                                        </span>
                                    </button>
                                ))}
                            </div>
                            <div className="p-6 bg-slate-900 dark:bg-slate-800 rounded-[32px] text-white/50 text-[10px] leading-relaxed italic">
                                &quot;Usa los mockups para validar el contraste y legibilidad del icono en entornos reales antes de exportar.&quot;
                            </div>
                        </div>
                    )}
                            ...
                            <div className="grid grid-cols-1 gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                                <button
                                    ...
                                    className={`w-full p-4 rounded-3xl flex items-center justify-between transition-all ${
                                        glossyEffect ? "bg-white dark:bg-slate-800 border-2 border-amber-400" : "bg-slate-100 dark:bg-slate-800 border-2 border-transparent"
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-gradient-to-t from-transparent to-white/60 border border-white/20" />
                                        <span className={`text-sm font-bold ${glossyEffect ? "text-amber-600" : "text-slate-600 dark:text-slate-300"}`}>Efecto Glossy</span>
                                    </div>
                                    <div className={`w-10 h-6 rounded-full relative transition-colors ${glossyEffect ? "bg-amber-400" : "bg-slate-300 dark:bg-slate-600"}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${glossyEffect ? "left-5" : "left-1"}`} />
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-3xl flex items-center justify-between transition-all ${
                                        longShadow ? "bg-white dark:bg-slate-800 border-2 border-indigo-500" : "bg-slate-100 dark:bg-slate-800 border-2 border-transparent"
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <svg className="w-8 h-8 text-indigo-500" fill="currentColor" viewBox="0 0 24 24"><path d="M21 3L3 21" stroke="currentColor" strokeWidth="2"/><path d="M12 12l9 9" opacity="0.5" stroke="currentColor" strokeWidth="2"/></svg>
                                        <span className={`text-sm font-bold ${longShadow ? "text-indigo-600" : "text-slate-600 dark:text-slate-300"}`}>Sombra Larga</span>
                                    </div>
                                    <div className={`w-10 h-6 rounded-full relative transition-colors ${longShadow ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-600"}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${longShadow ? "left-5" : "left-1"}`} />
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-3xl flex items-center justify-between transition-all ${
                                        showSafeGuide ? "bg-white dark:bg-slate-800 border-2 border-rose-500" : "bg-slate-100 dark:bg-slate-800 border-2 border-transparent"
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 border-2 border-rose-500 border-dashed rounded-lg" />
                                        <span className={`text-sm font-bold ${showSafeGuide ? "text-rose-600" : "text-slate-600 dark:text-slate-300"}`}>Guías Safe-Area</span>
                                    </div>
                                    <div className={`w-10 h-6 rounded-full relative transition-colors ${showSafeGuide ? "bg-rose-500" : "bg-slate-300 dark:bg-slate-600"}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${showSafeGuide ? "left-5" : "left-1"}`} />
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-3xl flex items-center justify-between transition-all ${
                                        glassOverlay ? "bg-white dark:bg-slate-800 border-2 border-pink-500" : "bg-slate-100 dark:bg-slate-800 border-2 border-transparent"
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-white/80 to-white/20 backdrop-blur-md shadow-inner border border-white/50" />
                                        <span className={`text-sm font-bold ${glassOverlay ? "text-pink-600" : "text-slate-600 dark:text-slate-300"}`}>Glassmorphism</span>
                                    </div>
                                    <div className={`w-10 h-6 rounded-full relative transition-colors ${glassOverlay ? "bg-pink-500" : "bg-slate-300 dark:bg-slate-600"}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${glassOverlay ? "left-5" : "left-1"}`} />
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-3xl flex items-center justify-between transition-all ${
                                        previewBackground === 'checker' ? "bg-slate-900 dark:bg-white text-white dark:text-slate-900" : "bg-slate-100 dark:bg-slate-800 text-slate-600"
                                    }`}
                                >
                                    <span className="text-sm font-bold">Fondo Checkerboard</span>
                                    <div className={`w-10 h-6 rounded-full relative transition-colors ${previewBackground === 'checker' ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${previewBackground === 'checker' ? "left-5" : "left-1"}`} />
                                    </div>
                                </button>
                            </div>
                        </div>
                    ...
                    {activeSection === "Export" && (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Descarga Rápida</label>
                                <div className="grid grid-cols-2 gap-2">
                                    <button onClick={() => handleDownload('png')} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 p-4 rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                        PNG {iconSize}px
                                    </button>
                                    <button onClick={handleExportSVG} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 p-4 rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-sm">
                                        VECTOR SVG
                                    </button>
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 block">Kit Completo Web / PWA</label>
                                <button 
                                ...
                                >
                                    ...
                                    <div className="text-center">
                                        <div className="font-black tracking-tight text-base">BUNDLE PWA & FAVICON</div>
                                        <div className="text-[9px] opacity-70 mt-1 uppercase tracking-widest">Incluye Manifest + 192px + 512px + ICO</div>
                                    </div>
                                    {isExporting && (
                                        <div className="absolute inset-x-0 bottom-0 h-1.5 bg-white/20">
                                            <div className="h-full bg-white shadow-[0_0_10px_white] transition-all duration-300" style={{ width: `${exportProgress}%` }} />
                                        </div>
                                    )}
                                </button>
                            </div>
                            ...
                            <div className="p-6 bg-emerald-500/5 border border-emerald-500/20 rounded-[32px]">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-wider">Optimizado para SEO</span>
                                </div>
                                <p className="text-[10px] text-slate-500 italic">
                                    Generamos automáticamente los archivos para cumplir estándares de Google PWA y Apple iOS.
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    >
                        <span className="text-xs font-bold uppercase tracking-tight">{darkMode ? "Modo Claro" : "Modo Oscuro"}</span>
                        {darkMode ? <span className="text-xl text-yellow-500">☀️</span> : <span className="text-xl text-indigo-400">🌙</span>}
                    </button>
                    <p className="text-[10px] text-center mt-4 text-slate-400 font-medium tracking-[0.2em] uppercase opacity-60">v3.0 • Advanced Design System</p>
                </div>
            </aside>

            {/* Main Workspace */}
            <main className="flex-1 relative bg-slate-100 dark:bg-slate-950 flex items-center justify-center overflow-hidden">
                {/* Background Decor */}
                <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
                    <div className="absolute top-[-10%] right-[-10%] w-[60%] h-[60%] bg-pink-500 rounded-full blur-[120px]" />
                    <div className="absolute bottom-[-10%] left-[-10%] w-[60%] h-[60%] bg-violet-600 rounded-full blur-[120px]" />
                </div>

                <div className={`relative z-10 transition-all duration-500 ${isExporting ? 'scale-90 opacity-50 blur-sm' : 'scale-100 opacity-100'}`}>
                    <div className="relative group">
                        {/* Status Badge */}
                        <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2">
                             <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/20 shadow-xl flex items-center gap-2 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                                <span className={`w-2 h-2 rounded-full ${contrastInfo.isGood ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'}`} />
                                <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-300 tracking-wider">
                                    {contrastInfo.isGood ? 'Contraste Pro' : 'Bajo Contraste'} • {iconSize}px
                                </span>
                            </div>
                        </div>
                        ...
                        {/* Preview Container */}
                        <div 
                            className={`p-12 rounded-[60px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] transition-all duration-500 relative overflow-hidden ${
                        ...
                        >
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm border transition-all ${
                                            previewBackground === btn.id 
                                            ? "bg-white dark:bg-slate-800 border-pink-500 shadow-lg scale-110" 
                                            : "bg-white/50 dark:bg-slate-800/50 border-white/20 hover:bg-white dark:hover:bg-slate-800"
                                        }`}
                                        title={`Fondo: ${btn.id}`}
                                    >
                                        {btn.icon}
                            ...
                            {showSafeGuide && (
                                <div className="absolute inset-x-0 inset-y-0 m-auto w-[280px] h-[280px] border-2 border-dashed border-rose-500/30 rounded-[80px] pointer-events-none z-50 animate-pulse" />
                            )}
                            {selectedMockup === "iphone" ? (
                                <div className="w-[300px] h-[600px] border-[8px] border-slate-900 dark:border-slate-800 rounded-[50px] relative overflow-hidden bg-[#222] shadow-2xl">
                                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-slate-900 rounded-b-2xl z-20" />
                                    <div className="absolute inset-0 bg-gradient-to-b from-blue-400 to-indigo-600 opacity-20" />
                                    <div className="mt-20 grid grid-cols-4 gap-4 p-4">
                                        {[...Array(12)].map((_, i) => (
                                            <div key={i} className={`w-full aspect-square rounded-2xl ${i === 2 ? "" : "bg-white/20 backdrop-blur-md animate-pulse"}`}>
                                                {i === 2 && (
                                                    <div className="w-full h-full scale-[0.8] drop-shadow-lg" dangerouslySetInnerHTML={{ __html: renderPreview.replace('width="400"', 'width="100%"').replace('height="400"', 'height="100%"') }} />
                                                )}
                                            </div>
                                        ))}
                                    </div>
                                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full border-2 border-white/20" />
                                </div>
                            ) : selectedMockup === "macos" ? (
                                <div className="w-[600px] h-[300px] flex items-end justify-center pb-8 bg-gradient-to-br from-orange-400 to-rose-500 rounded-3xl relative overflow-hidden">
                                     <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]" />
                                     <div className="flex gap-4 p-4 bg-white/20 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl scale-125">
                                         {[...Array(5)].map((_, i) => (
                                             <div key={i} className={`w-12 h-12 rounded-xl ${i === 2 ? "" : "bg-white/30"}`}>
                                                 {i === 2 && (
                                                    <div className="w-full h-full scale-[1.1] drop-shadow-xl" dangerouslySetInnerHTML={{ __html: renderPreview.replace('width="400"', 'width="100%"').replace('height="400"', 'height="100%"') }} />
                                                 )}
                                             </div>
                                         ))}
                                     </div>
                                ...
                                <div className="w-[500px] bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700">
                                    <div className="flex items-center gap-2 p-3 bg-slate-200 dark:bg-slate-900">
                                        <div className="flex gap-1.5">
                                            <div className="w-3 h-3 rounded-full bg-rose-500" />
                                            <div className="w-3 h-3 rounded-full bg-amber-500" />
                                            <div className="w-3 h-3 rounded-full bg-emerald-500" />
                                        </div>
                                        <div className="ml-4 flex items-center gap-2 px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-[10px] text-slate-500 font-bold min-w-[200px]">
                                            <div className="w-4 h-4" dangerouslySetInnerHTML={{ __html: renderPreview.replace('width="400"', 'width="100%"').replace('height="400"', 'height="100%"') }} />
                                            Mi Nueva App - Dashboard
                                        </div>
                                    </div>
                                    <div className="h-40 bg-white dark:bg-slate-950 p-6">
                                        <div className="w-1/2 h-4 bg-slate-100 dark:bg-slate-800 rounded-full mb-4" />
                                        <div className="w-3/4 h-2 bg-slate-50 dark:bg-slate-900 rounded-full mb-2" />
                                        <div className="w-2/3 h-2 bg-slate-50 dark:bg-slate-900 rounded-full" />
                                    </div>
                                </div>
                            ) : selectedMockup === "social" ? (
                                <div className="w-[400px] bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800">
                                    <div className="relative h-24 bg-gradient-to-r from-pink-400 to-indigo-500" />
                                    <div className="px-6 pb-6 mt-[-40px] relative">
                                        <div className="w-24 h-24 rounded-full border-4 border-white dark:border-slate-900 overflow-hidden bg-white dark:bg-slate-800 shadow-xl">
                                            <div className="w-full h-full scale-[1.2]" dangerouslySetInnerHTML={{ __html: renderPreview.replace('width="400"', 'width="100%"').replace('height="400"', 'height="100%"') }} />
                                        </div>
                                        <div className="mt-4">
                                            <div className="h-6 w-1/3 bg-slate-100 dark:bg-slate-800 rounded-lg mb-2" />
                                            <div className="h-3 w-1/5 bg-slate-50 dark:bg-slate-900 rounded-lg" />
                                        </div>
                                        <div className="flex gap-4 mt-6">
                                            <div className="flex-1 h-3 bg-slate-50 dark:bg-slate-900 rounded-full" />
                                            <div className="flex-1 h-3 bg-slate-50 dark:bg-slate-900 rounded-full" />
                                            <div className="flex-1 h-3 bg-slate-50 dark:bg-slate-900 rounded-full" />
                                        </div>
                                    </div>
                    ...
                    <div className="mt-12 flex items-center justify-center gap-4">
                        <button onClick={() => setIconText("")} className="px-6 py-3 rounded-2xl bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-white/20 dark:border-slate-800 text-slate-600 dark:text-slate-300 font-bold text-xs hover:bg-white dark:hover:bg-slate-900 transition-all flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            LIMPIAR
                        </button>
                        <button onClick={() => handleDownload('png')} className="px-8 py-4 rounded-3xl bg-pink-500 text-white font-black text-xs shadow-2xl shadow-pink-500/40 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 tracking-widest">
                ...
                {/* Progress Overlay */}
                {isExporting && (
                    <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
                        <div className="bg-white dark:bg-slate-900 p-12 rounded-[48px] shadow-2xl flex flex-col items-center gap-6 max-w-sm w-full border border-white/10">
                            <div className="relative w-24 h-24">
                                <svg className="w-full h-full rotate-[-90deg]">
                                    ...
                                    <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="251.2" strokeDashoffset={251.2 - (251.2 * exportProgress) / 100} className="text-pink-500 transition-all duration-300" strokeLinecap="round" />
                                </svg>
                            ...
                            <div className="text-center">
                                <h3 className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tight">Procesando Lote</h3>
                                <p className="text-xs text-slate-500 mt-2 font-medium">Generando resoluciones multi-formato...</p>
                            </div>
                        </div>
                    </div>
                )}
